"""Benchmark variant of train_baseline_model.py: identical except that the point graph takes
its own suffix, the calendar comes from the data tensor when present, and the time-of-day period is
steps-per-day rather than a hardcoded 24 -- the three assumptions that tie the original to hourly
Seoul data. Used to reproduce published PEMS-BAY accuracy with the same model code (prep_pemsbay.py,
eval_pemsbay.py)."""
"""
...
from models_baselines import (MODELS as _MODELS_MAIN, ODInjectionWrapper,
                              MatureInjectionWrapper, CrossAttnInjectionWrapper)
try:  # STID/STGCN live in a second registry; merge so one entry point covers every model
    from models_baselines_extra2 import MODELS_EXTRA2 as _M2
except Exception:
    _M2 = {}
try:
    from models_baselines_extra import MODELS_EXTRA as _M3
except Exception:
    _M3 = {}
...
def get_args():
    ...
    p.add_argument("--loss_fn", type=str, default="smooth_l1",
                    choices=["smooth_l1", "geh", "combined", "masked_mae"],
                    help="smooth_l1 (default, Huber loss on normalized values), masked_mae (L1 on "
                         "normalized values, positions where the dataset's valid_mask is 0 -- "
                         "missing/sentinel-zero observations, e.g. PEMS-BAY -- excluded from the "
                         "mean; matches the published masked-MAE training convention this "
                         "pipeline did not previously apply at train time, only at eval time via "
                         "eval_pemsbay.py), geh (mean GEH "
    ...
    p.add_argument("--lr_milestones", type=str, default="",
                    help="comma-separated epoch numbers for MultiStepLR decay, e.g. '20,40'. "
                         "Empty (default) = no decay, matching this script's original behavior. "
                         "Published PEMS-BAY training protocols typically decay LR partway "
                         "through a longer training run; this pipeline previously had no "
                         "scheduler at all.")
    p.add_argument("--lr_gamma", type=float, default=0.1, help="multiplicative LR decay factor applied at each --lr_milestones epoch")
    ...
    p.add_argument("--graph_suffix", type=str, default="",
                   help="suffix for the point-graph npz, independent of --dataset_suffix")
...
def main():
    ...
    if args.task == "volume":
        ...
        vol = tt["volume"]
        graph_path = f"{GTS}/volume_point_graph{args.graph_suffix}.npz"
        mape_thresh = 10.0
        val_key = "volume"
    else:
        tt = np.load(f"{GTS}/speed_tensor{args.dataset_suffix}.npz", allow_pickle=True)
        vol = tt["speed"]
        graph_path = f"{GTS}/speed_point_graph{args.graph_suffix}.npz"
        mape_thresh = 1.0
        val_key = "speed"
    ...
    # Missing-observation mask (PEMS-BAY-style: missing readings are recorded as 0 -- see
    # prep_pemsbay.py's valid_mask=(speed != 0)). Absent for datasets that don't carry one
    # (e.g. Seoul), in which case every position is treated as valid. This mirrors the published
    # PEMS-BAY protocol's masked training/eval convention, which train_benchmark_model.py did NOT
    # previously apply at train time (only eval_pemsbay.py masked at scoring time) -- a real,
    # previously-unaddressed deviation: training on raw-zero-coded missing positions teaches the
    # model to sometimes predict near-zero for what is actually just missing data.
    if "valid_mask" in tt:
        mask_flat = tt["valid_mask"].transpose(1, 2, 0).reshape(n_days * n_hours, n_sensors).astype(np.float32)
    else:
        mask_flat = np.ones_like(vol_flat, dtype=np.float32)
    valid_t = torch.tensor(mask_flat, device=device)  # (T,S)
    ...
    if "days" in tt:
        ...
        days = np.load(f"{GTS}/od_tensor_full.npz", allow_pickle=True)["days"]
    ...
    time_feat = np.stack([
        np.sin(2*np.pi*hours_arr/n_hours), np.cos(2*np.pi*hours_arr/n_hours),
    ...
    def get_batch(idx_list, bs, shuffle=True, return_starts=False):
        ...
        for i in range(0, len(idx), bs):
            ...
            md = torch.stack([valid_t[s+P:s+P+Q] for s in batch])                   # (B,Q,S) -- 1=real obs, 0=missing
            ...
            if return_starts:
                yield x_in, xd, od_dec, td, od_enc, md, batch
                continue
            yield x_in, xd, od_dec, td, od_enc, md
    ...
    if args.model == "stid_fixed":
        # STIDFixedBaseline's discrete time-of-day embedding needs the real per-day step count --
        # this benchmark script's data can be non-hourly (PEMS-BAY is 5-min, n_hours=288 here
        # despite the variable name), unlike the hourly-Seoul-only train_baseline_model.py.
        model_kwargs["steps_per_day"] = n_hours
    ...
    sched = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=milestones, gamma=args.lr_gamma) if milestones else None
    ...
    def compute_loss(pred, target_n, mask=None):
        if args.loss_fn == "masked_mae":
            assert mask is not None
            denom = mask.sum().clamp(min=1.0)
            return ((pred - target_n).abs() * mask).sum() / denom
        if args.loss_fn == "geh":
    ...
    for epoch in range(args.epochs):
        model.train()
        tr_losses = []
        for x_in, xd, od_dec, td, od_enc, md in get_batch(train_idx, args.batch_size):
            opt.zero_grad()
            pred = run_model(x_in, od_dec, td, od_enc)  # (B,S,Q)
            loss = compute_loss(pred, xd.transpose(1, 2), md.transpose(1, 2))
            loss.backward()
        ...
        val_losses = []
        with torch.no_grad():
            for x_in, xd, od_dec, td, od_enc, md in get_batch(val_idx, args.batch_size, shuffle=False):
                pred = run_model(x_in, od_dec, td, od_enc)
                val_losses.append(compute_loss(pred, xd.transpose(1, 2), md.transpose(1, 2)).item())
    ...
    csv_rows = []
    with torch.no_grad():
        for x_in, xd, od_dec, td, od_enc, md, starts in get_batch(test_idx, args.batch_size, shuffle=False, return_starts=True):
            pred = run_model(x_in, od_dec, td, od_enc)  # (B,S,Q)
            if args.use_od_injection:
