...
def get_args():
    ...
    p.add_argument("--od_channels", type=int, default=1,
                    help="1 -> routed_od_signal{suffix}.npz's routed_forecast (outflow only). 2 -> routed_od_signal_v2.npz's [routed_outflow, routed_inflow], or two named signals if --dual_signal_a/b_suffix are set")
    p.add_argument("--dual_signal_a_suffix", type=str, default=None,
                    help="with --od_channels 2: first channel's routed_od_signal{suffix}.npz (overrides the outflow/inflow default)")
    p.add_argument("--dual_signal_b_suffix", type=str, default=None,
                    help="with --od_channels 2: second channel's routed_od_signal{suffix}.npz")
    p.add_argument("--routed_signal_suffix", type=str, default=None,
                    help="override the routed_od_signal filename suffix independently of --dataset_suffix "
                         "(which also selects traffic_tensor/od_tensor). E.g. '_modeshare' loads "
                         "routed_od_signal_modeshare.npz while still using traffic_tensor.npz. "
                         "Defaults to --dataset_suffix when unset.")
...
def main():
    ...
    volume = tt["volume"]  # (n_sensors, n_days, 24)
    ...
    if args.use_od_injection:
        ...
        if args.od_channels == 1:
            rs = np.load(f"{GTS}/routed_od_signal{rs_suffix}.npz", allow_pickle=True)
            assert list(rs["sensor_link_ids"]) == link_ids, "routed_od_signal sensor order must match traffic_tensor"
            channels = [rs["routed_forecast"]]  # (T, S) -- outflow only, via W
        elif args.dual_signal_a_suffix is not None:
            # Two independently-normalized named channels, e.g. a calendar-climatology signal and
            # its de-climatologized residual (build_od_residual_signal.py). Each is normalized by
            # ITS OWN train-window std in the loop below -- the point of this branch: bundled into
            # one raw signal and normalized together, the residual occupies only ~0.6% of the
            # combined variance (climatology correlates 0.997 with the real signal) and is swamped
            # by the calendar-scale component; injected as a separate channel it is rescaled to its
            # own unit variance instead.
            ra = np.load(f"{GTS}/routed_od_signal{args.dual_signal_a_suffix}.npz", allow_pickle=True)
            rb = np.load(f"{GTS}/routed_od_signal{args.dual_signal_b_suffix}.npz", allow_pickle=True)
            assert list(ra["sensor_link_ids"]) == link_ids and list(rb["sensor_link_ids"]) == link_ids
            channels = [ra["routed_forecast"], rb["routed_forecast"]]
        else:
            rs = np.load(f"{GTS}/routed_od_signal_v2.npz", allow_pickle=True)
            assert list(rs["sensor_link_ids"]) == link_ids, "routed_od_signal_v2 sensor order must match traffic_tensor"
            channels = [rs["routed_outflow"], rs["routed_inflow"]]  # each (T, S), via W and V resp.
        norm_channels = []
        for ch in channels:
