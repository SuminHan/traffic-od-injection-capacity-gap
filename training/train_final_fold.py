"""Same model as train_gts_od.py, parameterized for rolling-fold validation on od_tensor_full.npz
(explicit train/val/test day-index ranges instead of the fixed 22/4/5-day split)."""
import argparse, json, time, os, datetime as dt
import numpy as np
import torch
import torch.nn.functional as F

from train_gts_od import KR_HOLIDAYS, TIME_FEAT_DIM, DynamicEdgeWeight, MaskedGraphGRUCell, Seq2SeqOD, build_windows
...
def main():
    ...
    od, codes, days = d["od"], d["codes"], d["days"]  # od: (n_days,24,N,N)
    n_days, n_hours, N, _ = od.shape
    T = n_days * n_hours
    od_flat = od.reshape(T, N, N).astype(np.float32)

    # mask built from the TRAIN range only (no peeking at val/test OD volumes when deciding
    # which edges exist)
    train_hours = (args.train_lo * n_hours, args.train_hi * n_hours)
    ...
    print(f"N={N} nodes, {n_edges} edges, fold days train=[{args.train_lo},{args.train_hi}) "
          f"val=[{args.val_lo},{args.val_hi}) test=[{args.test_lo},{args.test_hi})")
    ...
    day_objs = [dt.datetime.strptime("20" + str(dd), "%Y%m%d").date() for dd in days]
