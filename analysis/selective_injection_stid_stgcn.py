"""
Extends the selective-injection (validation-gated per-sensor selector) experiment to STID and
STGCN, now that their 30-fold plain/od_simple sweep just finished (EXTRA2_MODELS_EXT30_SWEEP_ALL_DONE).
Same no-leakage protocol as selective_injection_all_combos.py: per fold, per sensor, choose
plain vs od using ONLY that fold's validation-window forward pass (fresh inference from the
saved best.pt, never the test window), then apply the fixed per-fold selector to that fold's
already-computed test predictions. GPU work here is inference-only (no training), so this is
safe to run alongside the still-active GTS and PDFormer/AGCRN/MTGNN training sweeps without
meaningfully competing for GPU memory (only a modest slowdown from shared compute).
"""
...
def build_model(args, A, n_sensors):
    ...
    base_model = MODELS_EXTRA2[args.model](A, Fday, n_sensors, H=args.hidden, q_len=args.q_len, p_len=args.p_len).to(device)
...
summary_df.to_csv(f"{GTS}/selective_injection_stid_stgcn_summary.csv", index=False)
print("\n\n" + "="*70)
print("SUMMARY (stid, stgcn)")
print("="*70)
print(summary_df.to_string(index=False))
print("\nwrote selective_injection_stid_stgcn_summary.csv")
