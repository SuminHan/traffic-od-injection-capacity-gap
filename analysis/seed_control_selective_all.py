...
# Architectures live in two registries with two matching training entry points.
EXTRA2_MODELS = {"stid", "stgcn"}

# Ordered cheapest-first (measured min/fold) so the answer arrives incrementally. STID leads because
# it is now the paper's strongest favorable architecture and so the most consequential to check.
DEFAULT_COMBOS = [
    "stid:volume", "stid:speed", "staeformer:volume", "stgcn:volume",
    "gwnet:volume", "gwnet:speed", "stgcn:speed", "gman:speed",
    "staeformer:speed", "dcrnn:volume",
]
...
def main():
    ...
    summary_path = f"{GTS}/seed_control_all_summary.csv"
    all_summary = []
