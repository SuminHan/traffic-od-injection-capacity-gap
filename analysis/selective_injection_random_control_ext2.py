...
added when selective injection was run for PDFormer/MTGNN/AGCRN -- same random-selection negative
control (200 per-fold random per-sensor masks, matched sensor count, compared to the real
validation-based selector's result), computing real_mean_pct directly from each combo's own
fold_results CSV. No GPU, reuses existing test_predictions.csv files.
"""
import json
import numpy as np
import pandas as pd
...
out_df.to_csv(f"{GTS}/selective_injection_random_control_ext2_summary.csv", index=False)
print("\n" + out_df.to_string(index=False))
print("\nwrote selective_injection_random_control_ext2_summary.csv")
