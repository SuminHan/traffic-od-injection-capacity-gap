"""
Generalizes selective_injection_experiment.py (which showed selective, validation-selected
per-sensor injection beats plain STAEformer/speed: -1.23% MSE, 26/30 folds, p=1.7e-6, reversing
the uniform-injection aggregate harm of +4.71%) to all 8 (model,task) combinations that have a
full 30-fold plain+od_simple matrix sweep saved on disk: {dcrnn,gwnet,gman,staeformer} x
{volume,speed}. GTS is excluded (only a 10-fold non-_ext30 run exists for it).

Same no-leakage protocol as before: per fold, per sensor, choose plain vs od using ONLY that
fold's validation-window forward pass (freshly computed here from the saved best.pt checkpoint),
then apply the resulting fixed-per-fold selector to that fold's already-computed TEST
predictions. Reports selective-vs-plain and uniform-od-vs-plain side by side for every combo, so
it's clear whether the STAEformer/speed reversal is general or a one-off.
"""
...
for model_name, task, results_file in COMBOS:
    ...
    for e in results:
        variant = e["variant"]
        by_fold.setdefault(e["fold_start"], {})[variant] = e
    # od variant key differs: staeformer saved as "od_simple", others as "od"
    od_key = "od_simple" if "od_simple" in list(by_fold[list(by_fold.keys())[0]].keys()) else "od"
...
summary_df.to_csv(f"{GTS}/selective_injection_all_combos_summary.csv", index=False)
print("\n\n" + "="*70)
print("FULL SUMMARY (all 8 combos)")
print("="*70)
print(summary_df.to_string(index=False))
print("\nwrote selective_injection_all_combos_summary.csv")
