"""
Extends the selective-injection (validation-gated per-sensor selector) experiment to GTS, now
that its 30-fold plain/od_simple sweep just finished (GTS_EXT30_SWEEP_ALL_DONE). Same no-leakage
protocol as selective_injection_all_combos.py: per fold, per sensor, choose plain vs od using
...
predictions. GTS is already in models_baselines.py's MODELS dict (GTSBaseline) -- it was excluded
from selective_injection_all_combos.py purely because its 30-fold sweep wasn't done yet, not for
any architectural reason. Inference-only (no training), safe to run alongside other live sweeps.
"""
...
summary_df.to_csv(f"{GTS}/selective_injection_gts_summary.csv", index=False)
print("\nwrote selective_injection_gts_summary.csv")
print("SELECTIVE_INJECTION_GTS_ALL_DONE")
