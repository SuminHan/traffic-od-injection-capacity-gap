"""
Multi-path extension of build_routing_weights.py's STATIC routing weight matrix W.
Full K-shortest-paths (Yen's algorithm) is intractable per-OD-pair at this scale (500 origins x
up to 500 destinations); instead this uses Monte Carlo perturbed-Dijkstra stochastic route
assignment (a standard traffic-assignment technique): for each origin, run N_PERTURB independent
Dijkstra passes with i.i.d. log-normal multiplicative noise on every edge weight, so each pass's
shortest-path tree usually agrees with the deterministic one but occasionally diverges at close
ties or genuinely-competitive alternate routes. A destination's OD share gets split FRACTIONALLY
across whichever sensor links were crossed in >=1 of the N_PERTURB perturbed trees, weighted by
how often each was crossed -- so a sensor on a close-second route gets a partial contribution
instead of zero, while the single most common route still gets the largest share.
Same road network / OD-share / sensor-snap setup as build_routing_weights.py (unchanged), only
the per-origin routing loop is repeated under perturbation and averaged.
"""
import time
import numpy as np
...
N_PERTURB = 5
NOISE_SIGMA = 0.15  # log-normal multiplicative noise stddev on edge weights per perturbed pass
...
base_edge_w = length_w * (base_w ** ALPHA)  # deterministic base weight, perturbed per pass below
...
for oi, o_code in enumerate(mapped_codes):
    ...
    # N_PERTURB independent perturbed-weight Dijkstra passes from this origin
    pass_predecessors = []
    ...
    for d_code, d_i in code_to_nodeidx.items():
        ...
        if share <= 0:
            continue
        sensor_hit_count = {}  # sensor_col -> how many of N_PERTURB passes crossed it
        ...
        for sj, cnt in sensor_hit_count.items():
            W[row_out, sj] += share * (cnt / N_PERTURB)  # fractional credit by crossing frequency
    if oi % 50 == 0:
...
np.savez_compressed(f"{GTS}/routing_weights_multipath.npz", W=W, codes=np.array(codes_arr),
                     sensor_link_ids=np.array(sensor_link_ids), alpha=ALPHA,
                     n_perturb=N_PERTURB, noise_sigma=NOISE_SIGMA, snap_dist_m=dists)
print(f"saved routing_weights_multipath.npz, total {time.time()-t0:.0f}s")
