"""Apply routing_weights_speed.npz's W to the OD model's forecast (od_signal_ext.npz), producing
...
np.savez_compressed(f"{GTS}/routed_od_signal_speed.npz",
                     routed_forecast=routed.astype(np.float32),
                     sensor_link_ids=rw["sensor_link_ids"])
print(f"saved routed_od_signal_speed.npz: {routed.shape}")
