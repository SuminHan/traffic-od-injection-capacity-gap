"""Extended version of build_speed_tensor.py, reading THIS project's own speed_hourly_cache_ext.npz
(2023-01-01..2026-07-31, 43mo) instead of smhan's un-extended original. Same interpolation/valid-mask
...
np.savez_compressed(
    f"{GTS}/speed_tensor_ext.npz",
    speed=spd_tensor.astype(np.float32),
    valid_mask=valid_mask.reshape(n_sensors, n_days, 24),
    link_ids=np.array(link_ids),
    days=day_strs,
)
print(f"saved speed_tensor_ext.npz: speed {spd_tensor.shape}, days {day_strs[0]}..{day_strs[-1]}, "
