...
# MODE-SHARE VARIANT: raw OD movement counts all trip modes (walk/bike/transit/car); routing 100%
# of it onto the road network overweights short trips, which are disproportionately non-car in a
# dense transit-served metro. Discount each (origin,dest) pair's contribution by a distance-based
# car-mode-share BEFORE computing destination shares, using the same logistic (plateau=0.35,
# midpoint=2km) as the parallel simulator-project calibration -- uncalibrated against a real mode-
# share survey, but directionally standard and kept identical across both uses for consistency.
n_days, n_hours, N, _ = od["od"].shape
train_od_sum = od["od"][:365].sum(axis=(0, 1)).astype(np.float64)  # (N,N) total 2023 flow
codes_arr = our_codes
...
np.savez_compressed(f"{GTS}/routing_weights_modeshare.npz", W=W, codes=np.array(codes_arr),
                     sensor_link_ids=np.array(sensor_link_ids), alpha=ALPHA,
                     snap_dist_m=dists)
print(f"saved routing_weights_modeshare.npz, total {time.time()-t0:.0f}s")
