"""Apply the static routing_weights_modeshare.npz matrix W (500 dongs x 121 sensors) to per-node outflow
to get a properly ROUTED traffic signal per sensor -- both from ground-truth OD (for any
legitimate use as a known/past feature) and from the OD model's own forecast (od_signal.npz, for
...
assert rw_codes == od_codes, "routing_weights_modeshare.npz codes must align 1:1 with od_tensor_full.npz codes"
...
np.savez_compressed(f"{GTS}/routed_od_signal_modeshare.npz",
...
print(f"saved routed_od_signal_modeshare.npz: routed_actual {routed_actual.shape}, routed_forecast {routed_forecast.shape}")
